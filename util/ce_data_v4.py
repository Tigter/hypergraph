...
def load_data():
    ...
    graph_info = torch.load("/home/skl/yl/ce_project/relation_cl/pre_handle_data/ce_data_double_base_v4_graph_info.pkl")
    train_info = torch.load("/home/skl/yl/ce_project/relation_cl/pre_handle_data/ce_data_double_base_v4_train_info.pkl")
...
def build_graph_sampler(config):
    ...
    on_train_t = torch.utils.data.DataLoader(NagativeSampleDataset(graph_info["single_train"],
                                                                    ...
                                                                    graph_info['c2id'],graph_info['e2id'],toNodeSampler
                                                                    ),
            batch_size=config["batch_size"],
            shuffle=True, 
...
class NagativeSampleDataset(torch.utils.data.Dataset):
    def __init__(self, triples, nentity, nrelation, negative_sample_size, c2id, e2id,toNodeSampler):
        ...
        self.toNodeSampler = toNodeSampler
    ...
    def __getitem__(self, idx):
        ...
        sample_h = torch.LongTensor([head])
        ...
        return sample_h, sample_t[shuffle_idx], self.lable[shuffle_idx], self.toNodeSampler
    
    @staticmethod
    def collate_fn(data):
        sample_h = torch.cat([_[0] for _ in data], dim=0)
        sample_t = torch.stack([_[1] for _ in data], dim=0)
        lable = torch.stack([_[2] for _ in data], dim=0)
        toNodeSampler = data[0][3]
        return sample_h, sample_t, lable, toNodeSampler.sample(sample_h)
    
class ToNodeSampler(torch.utils.data.DataLoader):
    ...
    def __init__(self, graph_info,train_info,node_idx,batch_size=128,size=[2,2],n_size=10,mode="train",config=None,**kwargs):
        ...
        if mode == 'train':
            ...
            self.v2e_train_index = self.graph_info["v2e_train_index"]

        else:
            self.edge_index = self.graph_info["valid_edge_index"]
            self.traj2traj_edge_type = self.graph_info["valid_edge_type"]
            self.v2e_train_index = self.graph_info["v2e_valid_index"]
        ...
        self.e2v_adj_t = SparseTensor(
            row=self.v2e_train_index[0],
            col=self.v2e_train_index[1],
            value=torch.arange(self.v2e_train_index.size(1)),
            sparse_sizes=(self.num_nodes, self.num_nodes)
        ).t()
        ...
        super(ToNodeSampler, self).__init__(node_idx.view(-1).tolist(), collate_fn=self.sample,batch_size=batch_size,**kwargs)
    ...
    def sample(self, batch):
        ...
        for i, size in enumerate(self.sizes):
            if i == len(self.sizes) - 1:
                ...
                adj_t, n_id = self.e2v_adj_t.sample_adj(batch, size, replace=False)
                row, col, e_id = adj_t.coo()
                edge_attr = None
                edge_type = None
            else:
...
class CEGraphSampler(torch.utils.data.DataLoader):
    ...
    def sample(self, batch):
        ...
        out = (n_id , adjs, lable - self.c_num,split_idx)
