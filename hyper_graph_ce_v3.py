...
def load_interaction_data():
    ...
    # with open('./data/kegg/reaction_data/base_dict.pkl', 'rb') as fi:
    ...
    for c_list, e in reaction:
        ...
        for c in c_list:
            ...
            if e not in c2eList[c2id[c]]:
                print("error")
            pred_interaction_[c2eList[c2id[c]]] = pred_interaction_[c2eList[c2id[c]]] - 1 # 预测的score
...
if __name__=="__main__":
    ...
    # 读取数据集
    train_iter,c2eList,sampler,valid_sampler, test_sampler,graph_info,train_info,node_emb = build_graph_sampler(modelConfig)
    ...
    # 设置学习率更新策略
    lr_scheduler = MultiStepLR(optimizer,milestones=[2000], gamma=decay)
    logsInstance = []
    logsTypeOf= []
    logsSubOf = []
    logAll = []
