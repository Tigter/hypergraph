...
def parse_data(act_dir, mol_dir, protein_dir, with_label=True, prot_len=2500, prot_vec="Convolution", mol_vec="Convolution", mol_len=2048):
    ...
    if with_label:
        ...
        return {"protein_feature": protein_feature, "mol_feature": torch.tensor(mol_feature, dtype=torch.float32), "label": torch.tensor(label, dtype=torch.float32)}
    else:
...
class MolEnzPrediction(nn.Module):
    ...
    def predict(self, mol_feature, protein_feature):
        self.eval()
        with torch.no_grad():
            output = self(torch.tensor(mol_feature, dtype=torch.float32),
                          torch.tensor(protein_feature, dtype=torch.float32))
        return output.numpy()
...
def main():
    ...
    # act_prediction_model = tf.keras.models.load_model('/home/skl/yl/ce_project/EnzRank-main/split_0.model')
    ...
    test_dic = {test_name: parse_data(test_act, test_mol, test_protein, with_label=True, **type_params) for test_name, test_act, test_mol, test_protein in test_sets}
    valid_dic = {valid_name: parse_data(valid_act, valid_mol, valid_protein, with_label=True, **type_params) for valid_name, valid_act, valid_mol, valid_protein in valid_sets}
