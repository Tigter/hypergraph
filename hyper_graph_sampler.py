...
from util.sampler import GraphSampler,RelationSampler

import random
from core.HyperGraphV1 import HyperGraphV2
...
if __name__=="__main__":
    ...
    graph_info,v2e_edge_index,edge_index,edge_type, ht2edge_id, edgeInfo, data, valid_data, e2rel, neg_info,test_and_valid_neg_info = build_triple_entity_as_edge(on.train, on.nentity, on.nentity+on.nrelation,on.valid,on.test,modelConfig)

    # edge_index 顺序需要考虑一下
    all_node = on.nentity + on.nrelation + graph_info["hyper_edge_num"]
    
    node_feature =  nn.Embedding(on.nentity+on.nrelation, dim)

    rel_emb = node_feature.weight[on.nentity:on.nentity+on.nrelation]

    hyper_edge_feature =  nn.Embedding(graph_info["hyper_edge_num"], dim)
   
    train_id_list,valid_id_list,test_id_list,labels = data
   
    train_id_list = torch.LongTensor(train_id_list)
    valid_id_list = torch.LongTensor(valid_id_list)
    test_id_list = torch.LongTensor(test_id_list)
    ...
    dataset = {
        "x" : [node_feature.weight, hyper_edge_feature.weight],
        "edge_index" : [v2e_edge_index, edge_index],
        "hyper_edge_type": edge_type,
        "n_relation": on.nrelation ,
        "n_entity": on.nentity,
        "n_hyperedge": graph_info["hyper_edge_num"] ,
        "ht2edge_id": ht2edge_id,
        "train_id_list": train_id_list,
        "valid_id_list": valid_id_list,
        "test_id_list": test_id_list,
        "labels": labels,
        "train_max_he_id":graph_info["max_edge_id_train"],
        "e2rel_index": e2rel,
        "neg_info": neg_info
    }

    dataloader = GraphSampler(
        dataset,train_id_list,
        batch_size = 1024,
        size = [10,2],
        n_size = 29
    )
    ...
    model = HyperGraphV2(hyperkgeConfig=hyperConfig,n_node=on.nentity+on.nrelation,n_hyper_edge=graph_info["hyper_edge_num"])
    if cuda:
        model = model.cuda()
    ...
    optimizer = torch.optim.Adam([
        ...
        {
            'params':node_feature.weight,
        },
        {
            'params':hyper_edge_feature.weight,
        }
        ], lr=lr,
    )
    ...
    if args.train :
        ...
        for step in range(init_step, max_step):
            ...
            if step % test_step == 0 :
                ...
                logging.info('Valid Train num:%d at step: %d' % (len(train_id_list),step))

                # metrics = test_inductive(model,dataset,edgeInfo,train_id_list,rel_emb,'train')
                # logset.log_metrics('Train ',max_step, metrics)
                logging.info('Valid InstanceOf at step: %d' % step)
                metrics = test_inductive(model,dataset,valid_data,valid_id_list,test_and_valid_neg_info,'valid')
            ...
            for data in dataloader:
                log = HyperGraphV2.train_step(model=model,optimizer=optimizer,data=data,rel_emb=rel_emb,loss_funcation=base_loss_funcation)
                lr_scheduler.step()
                baselog.append(log)
            if step % 10 == 0:
        ...
        metrics = test_inductive(model,dataset,valid_data,test_id_list,test_and_valid_neg_info,'test')
        logset.log_metrics('Valid ',max_step, metrics)
